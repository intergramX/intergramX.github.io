(function (Scratch) {
  ...
  class IntergramXDataExt {
    ...
    async getData() {
      try {
        const response = await fetch('https://intergtamx-data-server.vercel.app/api/data');
        if (!response.ok) {
    ...
    async clearData() {
      try {
        const response = await fetch('https://intergtamx-data-server.vercel.app/api/data', { method: 'DELETE' });
        if (response.ok) {
          this.data = {};
          return 'Очищено';
        } else {
